...
FIXED = {
    "A_unused": replace(BASE, atr_stop_multiplier=2.0, use_support_stop=True,
                         cash_buffer_pct=0.0, use_regime_filter=False),
    "B_wide_stop": replace(BASE, atr_stop_multiplier=4.0, use_support_stop=False,
                           cash_buffer_pct=0.0, use_regime_filter=False),
    "C_immediate": replace(BASE, atr_stop_multiplier=4.0, use_support_stop=False,
                           cash_buffer_pct=0.0, use_regime_filter=True,
                           bull_exposure=1.0, bear_exposure=0.25, reentry_rule="immediate"),
    "R_confirm3": replace(BASE, atr_stop_multiplier=4.0, use_support_stop=False, cash_buffer_pct=0.0,
                          use_regime_filter=True, bear_exposure=0.25, reentry_rule="confirm3"),
    "R_confirm5": replace(BASE, atr_stop_multiplier=4.0, use_support_stop=False, cash_buffer_pct=0.0,
                          use_regime_filter=True, bear_exposure=0.25, reentry_rule="confirm5"),
    "R_staged": replace(BASE, atr_stop_multiplier=4.0, use_support_stop=False, cash_buffer_pct=0.0,
                        use_regime_filter=True, bear_exposure=0.25, reentry_rule="staged"),
    "R_momentum": replace(BASE, atr_stop_multiplier=4.0, use_support_stop=False, cash_buffer_pct=0.0,
                          use_regime_filter=True, bear_exposure=0.25, reentry_rule="momentum"),
}


REENTRY_RULES = ("immediate", "confirm3", "confirm5", "staged", "momentum")


def build_phase11_grid() -> list[StrategyParams]:
    """Phase 13 grid: ONLY the re-entry rule varies.

    ATR (4.0), support stop (off), regime (200DMA) and bear exposure (25%) are
    frozen at the Phase 12 candidate-C values. Nothing else is swept — the phase
    has exactly one question, and re-opening the risk layer would reintroduce
    the multiple-comparisons problem Phase 12 just closed.
    """
    return [
        replace(BASE, atr_stop_multiplier=4.0, use_support_stop=False, cash_buffer_pct=0.0,
                use_regime_filter=True, bull_exposure=1.0, bear_exposure=0.25, reentry_rule=rule)
...
def main() -> None:
    ...
    record = {
        "experiment_id": "phase13_recovery_lag",
        "git_commit": commit,
    ...
    while True:
        ...
        logger.info("  regime=%s  NIFTY %.2f%%  A %.2f%%  B %.2f%%  C %.2f%%  S %.2f%%",
                    ...
                    entry["oos"]["C_immediate"]["metrics"].get("total_return_pct") or 0,
                    entry["oos"]["B_wide_stop"]["metrics"].get("total_return_pct") or 0,
                    entry["oos"]["R_momentum"]["metrics"].get("total_return_pct") or 0,
...
def _report(record: dict) -> None:
    ...
    print("\n" + "=" * 150)
    print("PHASE 13 — FOLD-LEVEL OUT-OF-SAMPLE RESULTS")
    print("=" * 150)
    ...
    for f in folds:
        ...
        for key in ("B_wide_stop", "C_immediate", "R_confirm3", "R_confirm5", "R_staged", "R_momentum", "S_selected"):
    ...
    print("=" * 150)
    a, b_, c = (_m([_g(f, k, "total_return_pct") for f in folds]) for k in ("C_immediate", "R_confirm3", "R_staged"))
    a_dd, b_dd, c_dd = (_m([_g(f, k, "max_drawdown_pct") for f in folds]) for k in ("C_immediate", "R_confirm3", "R_staged"))
    a_dn, b_dn, c_dn = (_m([_g(f, k, "downside_capture_pct") for f in folds]) for k in ("C_immediate", "R_confirm3", "R_staged"))
    ...
    print("=" * 150)
    for key in ("C_immediate", "R_confirm3", "R_confirm5", "R_staged", "R_momentum", "S_selected"):
    ...
    for regime, fs in sorted(buckets.items()):
        ...
        print(f"  {regime:<14}{len(fs):>6}"
              ...
              f"{str(avg([_g(x,'C_immediate','total_return_pct') for x in fs])):>9}"
              f"{str(avg([_g(x,'B_wide_stop','total_return_pct') for x in fs])):>9}"
              f"{str(avg([_g(x,'R_momentum','total_return_pct') for x in fs])):>9}"
              f"{str(avg([_g(x,'S_selected','total_return_pct') for x in fs])):>9}"
              f"{str(avg([_g(x,'R_momentum','downside_capture_pct') for x in fs])):>12}"
